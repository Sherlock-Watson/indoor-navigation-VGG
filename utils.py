...
def calibrate_magnetic_wifi_ibeacon_to_position(file_data_map):
    mwi_datas = {}
    for path_filename, path_data in file_data_map.items():
        ...
        posi_data = path_data.waypoint

        step_positions = compute_step_positions(acce_data, ahrs_data, posi_data)
        # visualize_trajectory(posi_data[:, 1:3], floor_plan_filename, width_meter, height_meter, title='Ground Truth', show=True)
        ...
        if ibeacon_data.size != 0:
            ...
            ibeacon_datas_list = split_ts_seq(ibeacon_data, sep_tss)
            for ibeacon_ds in ibeacon_datas_list:
                ...
                if target_xy_key in mwi_datas:
                    ...
                    #     'ibeacon': ibeacon_ds
                    # }
        sep_tss = np.unique(magn_data[:, 0].astype(float))
        magn_datas_list = split_ts_seq(magn_data, sep_tss)
...
def compute_steps(acce_data):
    ...
    acce_min = np.zeros((2,))
    # [0, 0]
    acce_binary = np.zeros((window_size,), dtype=int)
    # [22 * 0]
    acce_mag_pre = 0
    state_flag = 0
    ...
    for i in np.arange(0, np.size(acce_data, 0)):
        # acce_data: data in one file
        acce_data_row = acce_data[i, :]
        acce_mag = np.sqrt(np.sum(acce_data_row[1:] ** 2))
        # get the magnitude of the acce
        ...
        if acce_mag_filt_detrend > np.max([acce_mag_pre, mag_threshold]):
            # peak
            acce_binary = np.append(acce_binary, [1])
            acce_binary = np.delete(acce_binary, 0)
            ...
            acce_binary = np.append(acce_binary, [0])
            acce_binary = np.delete(acce_binary, 0)

        if (acce_binary[-1] == 0) and (acce_binary[-2] == 1):
            if state_flag == 0:
                acce_max[:] = acce_data_row[0], acce_mag_filt
                state_flag = 1
            elif (state_flag == 1) and ((acce_data_row[0] - acce_max[0]) <= interval_threshold) and (
                    acce_mag_filt > acce_max[1]):
                acce_max[:] = acce_data_row[0], acce_mag_filt
            elif (state_flag == 2) and ((acce_data_row[0] - acce_max[0]) > interval_threshold):
                acce_max[:] = acce_data_row[0], acce_mag_filt
                state_flag = 1
        ...
        step_flag = False
        if step_criterion == 2:
            if (acce_binary[-1] == -1) and ((acce_binary[-2] == 1) or (acce_binary[-2] == 0)):
                step_flag = True
        elif step_criterion == 3:
            if (acce_binary[-1] == -1) and (acce_binary[-2] == 0) and (np.sum(acce_binary[:-2]) > 1):
                step_flag = True
        else:
            if (acce_binary[-1] == 0) and acce_binary[-2] == -1:
                if (state_flag == 1) and ((acce_data_row[0] - acce_min[0]) > interval_threshold):
                    acce_min[:] = acce_data_row[0], acce_mag_filt
                    state_flag = 2
                    step_flag = True
                elif (state_flag == 2) and ((acce_data_row[0] - acce_min[0]) <= interval_threshold) and (
                        acce_mag_filt < acce_min[1]):
                    acce_min[:] = acce_data_row[0], acce_mag_filt
        if step_flag:
            step_timestamps = np.append(step_timestamps, acce_data_row[0])
            step_indexs = np.append(step_indexs, [i])
            step_acce_max_mins = np.append(step_acce_max_mins,
                                           [[acce_data_row[0], acce_max[1], acce_min[1], acce_std ** 2]], axis=0)
...
def compute_step_positions(acce_data, ahrs_data, posi_data):
    # Get steps
    step_timestamps, step_indexs, step_acce_max_mins = compute_steps(acce_data)
    # Heading
    headings = compute_headings(ahrs_data)
    stride_lengths = compute_stride_length(step_acce_max_mins)
    step_headings = compute_step_heading(step_timestamps, headings)
    rel_positions = compute_rel_positions(stride_lengths, step_headings)
    step_positions = correct_positions(rel_positions, posi_data)
